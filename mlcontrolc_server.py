...
config = {'save_file_name': 'config_file',
          'checkpoint_file_name': '15_actions.out',
          ...
          'min_lon': -60,
          'max_lon': 60,
          'min_lat_gen':50,#51 , 49
          'max_lat_gen':54,#54 , 56
          'min_lon_gen':2,#2 , 0
          'max_lon_gen':8,#8 , 10
          'min_dist': -2,
          'max_dist': 3000,
...
class MyCallbacks(DefaultCallbacks):
    def on_episode_end(self, worker: RolloutWorker, base_env: BaseEnv,
        ...
        episode.custom_metrics['nr_ac_landed'] = sum(value == 1 for value in episode.agent_rewards.values())
        episode.custom_metrics['nr_ac_out_of_bounds']= sum(value == -0.5 for value in episode.agent_rewards.values())
        # episode.custom_metrics['nr_ac_wrong_approach']= sum(value == -0.49 for value in episode.agent_rewards.values())
...
if __name__ == "__main__":
    ...
    if settings.multiagent:
        ...
        high_obs = np.array(
            [10, settings.max_lat, settings.max_lon, 360, settings.max_dist, 180])  # , 360, 1000, 360, 1000, 360])
    ...
    if True:
        trainer = CCTrainer(
            ...
            config={
                ...
                'num_workers': 0,
                'num_sgd_iter':10,
                'rollout_fragment_length' : 5000, #600
                ...
                #     [250000000, 0.00001],
                # ],
                'clip_param': 0.3, #was .3
                'kl_coeff':0,
                'kl_target': 0.03,
                "explore": False, #not settings.evaluate,
                'lambda': 0.95,
                'shuffle_sequences': True
            })
