...
class BlueSkyEnv(MultiAgentEnv):
    ...
    def __init__(self, env_config): #**kwargs):
        ...
        recdataflag = False
        recdata = None
        self.NodeID = 0# kwargs['NodeID']
        self.n_cpu = 0 #kwargs['n_cpu']
        self.cfgfile = ''
    ...
    def step(self, action):
        ...
        for n_agent in range(self.nr_agents):
            action_tot = normalizer(action[n_agent], 'NormToHDG', self.min_hdg, self.max_hdg)
            self.myclient.send_event(b'STACKCMD', self.id_agents[n_agent] + np.array2string(action_tot))
